...
void Canvas::OnDraw(CDC* pDC)
{
	...
	for (i = begin; i < end; ++i)
	{
		...
		{
		case trap:
			tempW1.Format(_T("平行边1=%lf，"),quad.t->Width());
			tempW2.Format(_T("平行边2=%lf，"),quad.t->Width2());
			tempH.Format(_T("高=%lf, 面积="),quad.t->Height());
			msg=tempW1;
			msg+=tempW2;
			msg+=tempH;
			break;
		...
		case diam: 
			tempW1.Format(_T("对角线1=%lf，"),quad.p->Width());
			tempH.Format(_T("对角线2=%lf, 面积="),quad.p->Height());
			msg=tempW1;
			msg+=tempH;
			break;

		default:
			tempW1.Format(_T("宽=%lf，"),quad.p->Width());
			tempH.Format(_T("高=%lf, 面积="),quad.p->Height());
			msg=tempW1;
			msg+=tempH;
		}
